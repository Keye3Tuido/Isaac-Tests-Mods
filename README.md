...
> 设计时，没有做到每次执行"stage"指令前都重开本局游戏，导致部分结果可能出错！
> edited 2026-07-20
...
9. BossDirectionCompare
对比测试同一种子下，携带指定道具与不携带道具时各楼层头目房位置是否一致。测试道具包括：塔罗牌残片(饰品102)、月亮(589)、巫毒娃娃的头(599)、刀把(626)。每个种子自动执行5轮（1轮基准+4轮分别带单个道具），对比并记录差异楼层。
